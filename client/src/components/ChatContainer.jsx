      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {
          messages.map((msg, idx) => (
            <div key={idx} className={`flex items-center justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
              {
                msg.image ?
                  (
                  ...
                  (
                    <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 ml-2 mr-2 break-all bg-violet-500/30 text-white ${msg.senderId === authUser._id ? 'rounded-br-none' : 'rounded-bl-none'
                      }`}>{msg.text}</p>
                  )
              }
