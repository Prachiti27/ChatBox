      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {
          messagesDummyData.map((msg, idx) => (
            <div key={idx} className={`flex items-center justify-end ${msg.senderId != '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>
              {
                msg.image ? 
                (
                  <img src={msg.image} className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-48 ml-2 mr-2' />
                ) : 
                (
                  <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 ml-2 mr-2 break-all bg-violet-500/30 text-white ${msg.senderId === '680f50e4f10f3cd28382ecf9' ? 'rounded-br-none' : 'rounded-bl-none'
                    }`}>{msg.text}</p>
                )
              }
